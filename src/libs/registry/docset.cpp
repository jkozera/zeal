                       ...
                       int *start, int *length)
{
    ...
    *start = -1;
    ...
    int groupCount = 0;
    int bestRecScore = -1;
    int bestRecStart = -1, bestRecLength = -1;
            ...
            if (needle[i] == haystack[j++]) {
                if (*start == -1) {
                    *start = j;  // first matched char
                    ...
                    int recStart, recLength, recScore;
                    matchFuzzy(
                        needle, needleLength, haystack+j, haystackLength-j, &recStart, &recLength
                    );
                    if (recStart != -1) {
                        recScore = scoreFuzzy(haystack, recStart, recLength);
                        if (recScore > bestRecScore) {
                            bestRecScore = recScore;
                            bestRecStart = recStart;
                            bestRecLength = recLength;
                        }
                    }
                }
